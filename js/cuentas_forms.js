	...
	this.documentoMask = function ( tipoDocumentoid, nroDocumentoid, incluirguion, tipoDocumentoDNI, tipoDocumentoNIE){
		...
		if( tipoDoc == tipoDocumentoDNI ){//dni
			...
			$(nroDocumentoid).inputmask({
		        mask: ["9999999[9]" + guion + "A"], placeholder: ""
		    });
		}else if( tipoDoc == tipoDocumentoNIE ){//nie
			$(nroDocumentoid).inputmask({
		        mask: "X-9999999[9]" + guion + "A", placeholder: ""
		    });
		}else{
			$(nroDocumentoid).inputmask({
		        mask: ["99999999"], placeholder: ""
		    });
		}    
...
$(document).ready(function() {
	...
	$(".input-text").not(".input-text-area, .input-email, .input-percentage, .input-date, .input-number, .input-currency").each(function( index ) {
